...
import { applyQuickForwarder, loadQuickList, type QuickForwarder } from '../../../lib/quick-lists'
...
export function ProxyForwarders({ f, set, en }: PaneProps) {
  ...
  */
  ...
  useEffect(() => {
    let alive = true
    void loadQuickList<QuickForwarder>('quick-forwarders-list').then((e) => alive && setQuick(e))
    return () => {
      alive = false
    }
  }, [])
  ...
  return (
    <>
      ...
      <Block title="Forwarders">
        ...
        />
        {/* Upstream calls it "Quick Select" here and "Quick Add" on Blocking.
            Both names are its own and travel verbatim. */}
          ...
          {(id) => (
            <Select
              ...
              onChange={(ev) => {
                ...
                if (entry) set(applyQuickForwarder(entry))
              }}
            >
              <option value="" />
