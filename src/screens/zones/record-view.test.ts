...
describe('the Data cell by type', () => {
  ...
  it('FWD only shows the proxy when the type has one', () => {
    const without = pairs(rec('FWD', { protocol: 'Udp', forwarder: '1.1.1.1', priority: 1, dnssecValidation: true, proxyType: 'DefaultProxy' }))
    expect(without).not.toHaveProperty('Proxy Address:')
    const withIt = pairs(rec('FWD', { protocol: 'Udp', forwarder: '1.1.1.1', priority: 1, dnssecValidation: true, proxyType: 'Socks5', proxyAddress: '10.0.0.9', proxyPort: 1080 }))
    expect(withIt['Proxy Address:']).toBe('10.0.0.9')
  })
...
describe('the footer of the cell', () => {
  ...
  it('the expiry only appears with expiryTtl > 0', () => {
    ...
    const withIt = recordFooter(rec('A', {}, { expiryTtl: 3600, expiryTtlString: '1h' }), NOW)
    expect(withIt.find((x) => x.label === 'Expiry TTL:')?.value).toBe('3600 (1h)')
  })
})

describe('the name relative to the zone', () => {
  it('the apex is @', () => {
    expect(relativeName('casa.test', 'casa.test')).toBe('@')
  })
  it('a sub-domain loses the suffix', () => {
    expect(relativeName('www.casa.test', 'casa.test')).toBe('www')
    expect(relativeName('a.b.casa.test', 'casa.test')).toBe('a.b')
  })
...
describe('hiding the DNSSEC records', () => {
  it('removes the five types, not six', () => {
    ...
    expect(hideDnssec(list).map((r) => r.type)).toEqual(['A', 'ZONEMD'])
  })
})
