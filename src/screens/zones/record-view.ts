...
import { dateWithAge, dateTime, minuteStamp } from '../../lib/dates'
  ...
  switch (r.type.toUpperCase()) {
    ...
    case 'DNSKEY': {
      ...
      if (d.dnsKeyState != null) {
        ...
        if (d.dnsKeyStateReadyBy != null) state += ` (ready by: ${minuteStamp(s(d.dnsKeyStateReadyBy))})`
        else if (d.dnsKeyStateActiveBy != null) state += ` (active by: ${minuteStamp(s(d.dnsKeyStateActiveBy))})`
...
export function recordFooter(r: ResourceRecord, now?: number): { label: string; value: string }[] {
  ...
  if (r.expiryTtl > 0) {
    ...
    pairs.push({ label: 'Expires On:', value: dateWithAge(expires.toISOString(), now) })
  }

  // `0001-01-01T00:00:00` is "never", and there upstream does NOT put the age.
  const never = r.lastUsedOn === '0001-01-01T00:00:00'
  pairs.push({
    label: 'Last Used:',
    value: never ? `${dateTime(r.lastUsedOn)} (never)` : dateWithAge(r.lastUsedOn, now),
  })

  if (r.lastModified !== '0001-01-01T00:00:00' && r.lastModified !== '0001-01-01T00:00:00Z') {
    pairs.push({ label: 'Last Modified:', value: dateWithAge(r.lastModified, now) })
  }
...
export function hideDnssec(records: ResourceRecord[]): ResourceRecord[] {
...
*/
export {
  minuteStamp as shortDate,
  dateTime as longDate,
