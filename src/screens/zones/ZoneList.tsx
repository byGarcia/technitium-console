...
import { minuteStamp as date } from '../../lib/dates'
import { statusText, pageWindow } from '../../lib/pagination'
...
export function ZoneList({
  ...
  */
  ...
  const load = useCallback(
    async (page: number) => {
      ...
      if (r.kind !== 'ok') {
        /*
        A failure is reported ONCE, and who reports it depends on whether there
        was data.

        · **With previous data** the stale strip reports it, and it also says
        ...
        */
        ...
        setStale(hadData)
        if (!hadData) onNotice(noticeFromFailure(r))
        /* A first load that failed is no longer waiting: it has an answer, and it
           is a bad one. Leaving `loaded` false here would leave the screen saying
           "loading" over a failure that has already been reported. */
        setLoaded(true)
        return
      }
    ...
    [token, node, nameFilter, typeFilter, perPage, onNotice, lastGood],
  )
  ...
  */
  ...
  const state = statusText(firstRow, zones.length, totalZones, pageNumber, totalPages, 'zones')
  const pg = pageWindow(pageNumber, totalPages)
  const { rows: visibleZones, sort, toggle } = useSort(KEYS, zones)
  ...
  return (
    <>
      ...
      */}
      ...
      The bar is ALWAYS drawn and the button is always enabled, which is not what
      ...
      and another in the table header; two controls doing the same thing is what
      this console removes everywhere else, and the table's is the one the rows
      align with.
      */}
      <div className={styles.selection}>
        <span>
          {checkedOnes.length === 0
            ? state
            : `${checkedOnes.length} of ${zones.length} zones selected`}
        </span>
      ...
      <Table
        /* `tableClass` and not `className`: the first reaches the `<table>` and
           the second stays on the wrapper, which has no border. Measured: with
           `className` the `--dan` border did not appear. */
        tableClass={stale ? styles.stale : undefined}
        header={
          <>
      ...
      <div className={`${styles.count} ${styles.countFoot}`}>
        <span>{state}</span>
        {pagination}
      </div>
        </>
...
interface RowProps {
  ...
  index: number
  checked: boolean
  /** With stale data the row is not selectable. */
  stale: boolean
  busy: boolean
  canModify: boolean
  canDelete: boolean
      ...
      <td className={tbl.checkCell}>
        <label>
          <input
            ...
            /* With stale data there is no selecting: acting in bulk on a list
               that no longer reflects the server is how the wrong zone gets
               deleted. */
            disabled={p.stale}
            checked={p.checked}
