 ...
 * whatever `compileNameFilter` returns. It needs the zone because without
...
export function filterBy(records: ResourceRecord[], filter: Filter, zone: string): ResourceRecord[] {
  ...
  const { domain, regex } = compileNameFilter(name, zone)
  ...
  return records.filter((r) => {
    ...
    if (regex == null) {
      if (domain != null && recordName !== domain) return false
    } else if (!regex.test(recordName)) {
      return false
    }
