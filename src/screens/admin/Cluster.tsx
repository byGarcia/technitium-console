...
import { fromNow, minuteStamp } from './dates'
import {
  noticeFromFailure,
  Confirm,
  ForceBlock,
...
export function Cluster({ tabs, token, cluster, node: controlledNode, onNodeChange, onCluster, onNotice }: Props) {
  ...
  const { rows: visibleNodes, sort, toggle } = useSort(KEYS, nodes)
  ...
  return (
    <>
      {/*
      The role of THIS server, beside the title.

      Two of the three branches are the same nine-column table and differ only in
      which buttons the header carries, so without this you tell them apart by
      reading the toolbar. The word is not a new one: it is the `Type` of the row
      whose `State` is `Self`, the same literal the table shows in its own column.

      It is not drawn on the branch with no cluster: there the empty panel already
      says `Cluster Not Initialized` in full, and a pill would repeat half of it.
      */}
      <SectionHeader
        title="Administration"
        tabs={tabs}
        labels={
          initialised && ownType != null ? (
            <Tag tone="info">{ownType}</Tag>
          ) : undefined
        }
        actions={<>{initialised && !isPrimary && (
      ...
      {loading ? (
        ...
        <>
          ...
          >
            {visibleNodes.map((n) => (
              /* This node's own row, washed in `--acc-bg`. The `Self` in its
...
function RelativeDate({ iso }: { iso?: string }) {
  ...
  return <LastSeenCell date={minuteStamp(iso)} ago={fromNow(iso)} />
}
...
function NewCluster({
  ...
  async function inicializar() {
    ...
    setBusy(true)
    const outcome = await initCluster(token, domain, clean)
    setBusy(false)
...
function JoinCluster({
  ...
  return (
    <Dialog
      ...
      actions={
        <>
          <Button variant="primary" disabled={busy || loading || alreadyThere} onClick={() => void join()}>
            Join
          </Button>
        </>
      }
...
function ClusterOptions({
  ...
  useEffect(() => {
    ...
    void getClusterState(token, { node: node }).then((outcome) => {
      ...
      setIsPrimary(r.clusterNodes?.find((n) => n.state === 'Self')?.type === 'Primary')
...
function EditOwnNode({
  ...
  const [loading, setLoading] = useState(true)
  const [ips, setIps] = useState<string[]>([])
  const [list, setList] = useState(`${target.ipAddresses.join('\n')}\n`)
  const [notice, setNotice] = useState<Notice | null>(null)
  const [busy, setBusy] = useState(false)
  ...
  async function save() {
    ...
    setBusy(true)
    const outcome = await updateIpAddress(token, clean, node)
    setBusy(false)
...
function EditPrimaryNode({
  ...
  async function save() {
    ...
    let clean = cleanList(list)
    if (clean === ',') clean = ''

    setBusy(true)
    const outcome = await updatePrimaryNode(token, url, clean, node)
    setBusy(false)
...
function PromoteNode({
  ...
  return (
    <Dialog
      open
      onOpenChange={(o) => !o && onClose()}
      title={`Promote To Primary Node - ${target.name}`}
      actions={
        <>
          <Button variant="danger" disabled={busy} onClick={() => void promote()}>
            Promote
          </Button>
        </>
      }
...
function LeaveCluster({
  ...
  return (
    ...
    >
      ...
      <p className={styles.paragraph}>Are you sure you want to leave the Cluster?</p>
      <ForceBlock
...
function DeleteCluster({
  ...
  return (
    ...
    >
      ...
      <p className={styles.paragraph}>Are you sure you want to delete the Cluster?</p>
      <ForceBlock
