...
import { statusText, pageWindow } from './pagination'
...
describe('status text', () => {
  it('with items it states the range, the total and the page', () => {
    expect(statusText(11, 10, 47, 2, 5, 'zones')).toBe('11-20 (10) of 47 zones (page 2 of 5)')
  })

  it('with no items the whole sentence changes', () => {
    expect(statusText(1, 0, 0, 1, 1, 'records')).toBe('0 records')
  })
})
