...
export function minuteStamp(iso: string | null | undefined): string {
  if (iso == null) return ''
  const d = parts(iso)
  if (d == null) return ''
  return (
...
export function dateWithAge(iso: string, now?: number): string {
  return `${dateTime(iso)} (${fromNow(iso, now)})`
}
